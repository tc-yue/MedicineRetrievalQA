...
class WordSimilarity:
    ...
    def __init__(self):
        ...
        self.__e = 0.5
        self.__f = 0.1
        self.conn = sqlite3.connect(os.path.abspath(os.path.dirname(__file__)+os.path.sep+os.pardir)+'/files/cilin/cilin.db')
        self.cursor = self.conn.cursor()

    # 获取词的编码
    def get_encode(self, word1, word2):
        self.cursor.execute("select * from CILIN where word = ?", (word1,))
        results1 = self.cursor.fetchall()
        self.cursor.execute("select * from CILIN where word = ?", (word2,))
    ...
    # 计算所在分支层分支数目
    def get_n(self, encode_head):
        length = len(encode_head)
        self.cursor.execute("select * from CILIN where labels like ?", [encode_head+'%'])
...
if __name__ == '__main__':
    for i in ['村容','山色','景色','外贸','村毛']:
        print(WordSimilarity().get_similarity('风景', i))
